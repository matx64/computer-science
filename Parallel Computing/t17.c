...
// Tarefa 17 GRUPO 4 - Computação Paralela

/* 
    Tempo Sequencial:   0m2.634s

    Tempo Paralelo:     0m1.438s

    Os tempos com diferentes Schedule (static e dynamic) ficaram praticamente os mesmos.
...
int LCSubStr(char *x, char *y, int m, int n)
{
  ...
  for (int i = 0; i <= m; i++)
  {
    #pragma omp parallel for num_threads(2) reduction(max:result) schedule(static)
    for (int j = 0; j <= n; j++)
    {
      if (i == 0 || j == 0)
        LCSuff[i][j] = 0;
