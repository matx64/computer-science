...
class TP03Q02{
	public static void main(String[]args) throws Exception{
		...
		for(int i = 0; i < numEntrada; i++){
			...
			arvoreArvore.mostrarPre(analisePresidente(presidentes1[i]));
		}
		...
		for(int i = 0; i < numEntrada; i++){
			arvoreArvore.pesquisar(entrada2[i]);
		}

		// Escrevendo informações no log
		Arq.openWrite("636132_arvoreArvore.txt");
		long tempoExecucao = System.currentTimeMillis() - inicioExecucao;
		Arq.print("636132" + "\t" + tempoExecucao + "\t" + arvoreArvore.numComparacoes);
		Arq.close();
	}
