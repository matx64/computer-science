...
class TP03Q07{
	public static void main(String[]args) throws Exception{
		...
		for(int i = 0; i < numEntrada; i++){
			...
			hashIndireta.inserir(analisePresidente(presidentes1[i]));
		}
		...
		// Pesquisa e saídas
		for(int i = 0; i < numEntrada; i++){
			if(hashIndireta.pesquisar(entrada2[i])){
		...
		Arq.openWrite("636132_hashIndireta.txt");
		long tempoExecucao = System.currentTimeMillis() - inicioExecucao;
		Arq.print("636132" + "\t" + tempoExecucao + "\t" + hashIndireta.numComparacoes);
		Arq.close();
	}
