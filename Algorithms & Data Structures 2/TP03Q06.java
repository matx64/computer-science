...
class TP03Q06{
	public static void main(String[]args) throws Exception{
		...
		for(int i = 0; i < numEntrada; i++){
			...
			hashRehash.inserir(analisePresidente(presidentes1[i]));
		}
		...
		// Pesquisa e saídas
		for(int i = 0; i < numEntrada; i++){
			if(hashRehash.pesquisar(entrada2[i])){
		...
		Arq.openWrite("636132_hashRehash.txt");
		long tempoExecucao = System.currentTimeMillis() - inicioExecucao;
		Arq.print("636132" + "\t" + tempoExecucao + "\t" + hashRehash.numComparacoes);
		Arq.close();
	}
