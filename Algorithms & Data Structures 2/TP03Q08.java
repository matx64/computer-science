...
class TP03Q08{
	public static void main(String[]args) throws Exception{
		...
		// Entrada de presidentes da arvore 1
		do{
			entrada[numEntrada] = MyIO.readLine();
		}while(entrada[numEntrada++].equals("FIM") == false);
		numEntrada--;

		// Inserindo nomes dos presidentes na arvore 1
		Presidente[] presidentes1 = new Presidente[25];
		for(int i = 0; i < numEntrada; i++){
			presidentes1[i] = new Presidente();
			presidentes1[i].setPagina(entrada[i]);
			arvoreTrie1.inserir(analisePresidente(presidentes1[i]).getNome());
		}

		// Entrada de presidentes da arvore 2
		String[] entrada1 = new String[25];
		numEntrada = 0;
		do{
			entrada1[numEntrada] = MyIO.readLine();
		}while(entrada1[numEntrada++].contains("FIM") == false);
		numEntrada--;

		// Inserindo nomes dos presidentes na arvore 2
		Presidente[] presidentes2 = new Presidente[25];
		for(int i = 0; i < numEntrada; i++){
			presidentes2[i] = new Presidente();
			presidentes2[i].setPagina(entrada1[i]);
			arvoreTrie2.inserir(analisePresidente(presidentes2[i]).getNome());
		}
		...
		// Merge das duas arvores (A2 na A1)
		arvoreTrie1.merge(arvoreTrie2);

		// Pesquisa e saídas
		for(int i = 0; i < numEntrada; i++){

			if(arvoreTrie1.pesquisar(entrada2[i])){
				MyIO.println("SIM");
			} else{
		...
		Arq.openWrite("636132_arvoreTrie.txt");
		long tempoExecucao = System.currentTimeMillis() - inicioExecucao;
		Arq.print("636132" + "\t" + tempoExecucao + "\t" + arvoreTrie1.numComparacoes);
		Arq.close();
	}
