...
class TP03Q05{
	public static void main(String[]args) throws Exception{
		...
		int numEntrada = 0;
		HashReserva hashReserva = new HashReserva();
		...
		for(int i = 0; i < numEntrada; i++){
			...
			hashReserva.inserir(analisePresidente(presidentes1[i]));
		}
		...
		// Pesquisa e saídas
		for(int i = 0; i < numEntrada; i++){
			if(hashReserva.pesquisar(entrada2[i])){
				MyIO.println(entrada2[i] + " SIM");
			}else{
				MyIO.println(entrada2[i] + " N"+(char)195+"O");
			}
		}

		// Escrevendo informações no log
		Arq.openWrite("636132_hashReserva.txt");
		long tempoExecucao = System.currentTimeMillis() - inicioExecucao;
		Arq.print("636132" + "\t" + tempoExecucao + "\t" + hashReserva.numComparacoes);
		Arq.close();
	}
